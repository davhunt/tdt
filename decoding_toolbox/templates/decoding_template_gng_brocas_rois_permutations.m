function [] = decoding_template_gng_brocas_rois_permutations(subject_num, num_permutations, num_cv_splits, roi_mask_file, roi_name)
...
%%%%%%%%
    ...
    perm_results_dir = ['/Volumes/data1/Complex_seq_analysis/' num2str(subject_num, '%.3d') '/MVPA/durations_unsmoothed_native/TDT_results_brocas/' roi_name '_perm/perm' num2str(i_perm, '%.3d')];
    ...
    for spl = 1:num_cv_splits
        ...
        movefile(['/Volumes/data1/Complex_seq_analysis/' num2str(subject_num, '%.3d') '/MVPA/durations_unsmoothed_native/TDT_results_brocas/' roi_name '_perm/perm' num2str(i_perm, '%.3d') '/results_rep' num2str(spl, '%.2d') '/res_accuracy_minus_chance_' name '.nii'], ...
          ['/Volumes/data1/Complex_seq_analysis/' num2str(subject_num, '%.3d') '/MVPA/durations_unsmoothed_native/TDT_results_brocas/' roi_name '_perm/perm' num2str(i_perm, '%.3d') '/results_rep' num2str(spl, '%.2d') '/res_accuracy_minus_chance.nii']);
    end
end
...
tEnd = toc(tStart);
disp(['Running ' num2str(num_cv_splits) ' splits on subject ' num2str(subject_num) ' took ' num2str(tEnd, '%.2f') ' seconds. Done.'])
