...
% Notes:
% For contrasts "AP" or "CAOR" only use grammatical trials, while "GUG" use all trials
...
%U = T(matches(T.grammar_type, ["A","P"]),:);


%file_list = dir('/Volumes/data1/Implicit_Learning/sentence_reading_lss/01/windowType1/*beta*.nii');
%file_list = dir(fullfile(input_dir, subjChar, window_type, '*beta*.nii'));
%beta_files = {file_list.name}'; % cfg.files.name



%[filepath,name,ext] = fileparts(file_list(1).name);
%pattern = '^beta_([0-9]{2})([A-Z]+)$';
