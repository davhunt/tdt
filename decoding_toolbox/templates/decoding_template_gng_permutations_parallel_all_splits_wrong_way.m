...
for spl = 1:length(cv_splits)
    ...
    cfg_file = ['/Volumes/data1/Complex_seq_analysis/' num2str(subject_num, '%.3d') '/MVPA/durations_unsmoothed/TDT_results_run' num2str(cv_splits(spl), '%.2d') '/res_cfg.mat'];
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%% REMOVE
    %cfg.results.dir = '/Volumes/data1/Complex_seq_analysis/004/MVPA/durations_unsmoothed/TDT_results_run04';
    %%%%%%%%%%%%%%%5 REMOVE LATER
    cfg.results.dir = ['/Volumes/data1/Complex_seq_analysis/' num2str(subject_num, '%.3d') '/MVPA/durations_unsmoothed/TDT_results_run' num2str(cv_splits(spl), '%.2d')];
    ...
    n_perms = num_permutations;  % 10 chosen for demo only. Typically a you should pick large 
    ...
    %parfor i_perm = 55:n_perms
        cfg_copy = cfg;
        cfg_copy.plot_design = 0; % no plot with parfor
        cfg_copy.verbose = 0;
        cfg_copy.results.dir = [cfg.results.dir '/' num2str(i_perm, '%.3d')]; % works for 001-999, probably no more permutations than that
        if ~isfolder(cfg_copy.results.dir)
            mkdir(cfg_copy.results.dir)
        end
        dispv(1, 'Permutation %i/%i', i_perm, n_perms)
        
        cfg_copy.design = designs{i_perm};
        cfg_copy.results.filestart = ['perm' num2str(i_perm, '%.3d')];
        ...
        results = decoding(cfg_copy); % run permutation
        
        % rename  design figures to start with the current permutation number
        designfiles = dir(fullfile(cfg_copy.results.dir, 'design.*'));
        for design_ind = 1:length(designfiles)
            movefile(fullfile(cfg_copy.results.dir, designfiles(design_ind).name), ...
                        fullfile(cfg_copy.results.dir, [cfg_copy.results.filestart '_' designfiles(design_ind).name]));
        end
    end
    delete(gcp('nocreate'))
...
tEnd = toc(tStart);
disp(['Done. Job took ' num2str(tEnd, '%.2f') ' seconds.'])
