function [] = stats_average_repetitions_HL(reps_folder, type)
    ...
    for fsuf = ["","_H","_L","_H-L"]
        ...
        for i = 1:length(all_dirs)
            if regexp(all_dirs(i).name, '^TDT_results_rep[0-9]+$')
                nif = load_untouch_nii([reps_folder '/' all_dirs(i).name '/res_accuracy_minus_chance' char(fsuf) '.nii.gz']);
                if firstFlag
                    sz = size(nif.img);
                    firstFlag = 0;
        ...
        save_untouch_nii(nif, [reps_folder '/TDT_results_avg_allreps/res_accuracy_minus_chance' char(fsuf) '.nii.gz']);
    end
